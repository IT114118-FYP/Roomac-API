    ...
    public function getReservationsForTimeTable($resourceId, $start_date, $end_date) {
        ...
        for ($i = 0; $i < $diff; $i++) {
            ...
            foreach ($rrs as $rr) {
                ...
                $startTime = Carbon::parse($date.'T'.Carbon::parse($rr->start)->format('H:i:s'))->format('Y-m-d\TH:i:s');
                $endTime = Carbon::parse($date.'T'.Carbon::parse($rr->end)->format('H:i:s'))->format('Y-m-d\TH:i:s');
