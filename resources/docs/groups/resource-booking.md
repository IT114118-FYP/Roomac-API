...
```bash
curl -X GET \
    -G "https://it114118-fyp.herokuapp.com/api/resources/{resource}/bookings?start=eveniet&end=id" \
...
curl -X POST \
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"date":"ut","start":"veniam","end":"aut"}'
...
curl -X PUT \
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"date":"deleniti","start":"aut","end":"nemo"}'
